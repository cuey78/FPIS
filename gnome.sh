...
set_gtk_theme() {
    ...
    if ! rpm -q gnome-themes-extra &>/dev/null; then
        ...
        dnf install -y gnome-themes-extra gnome-tweaks
    fi
    
    # Set the GTK theme as the normal user
    if sudo -u "$current_user" DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$(id -u $current_user)/bus" gsettings set org.gnome.desktop.interface gtk-theme "$theme_name"; then
        dialog --msgbox "GTK theme successfully changed to $theme_name" 0 0
        return 0
    else
        dialog --msgbox "Error: Failed to set GTK theme to $theme_name" 0 0
        return 1
    fi
}
...
choose_gtk_theme() {
    ...
    CHOICE=$(dialog --clear --title "GTK Theme Selector" \
        --menu "Current theme: $current_theme\nChoose a GTK theme:" 17 45 5 \
        1 "Adwaita (Light)" \
        2 "Adwaita-dark (Dark)" \
        3 "HighContrast (Accessibility)" \
        4 "Raleigh (Legacy)" \
        5 "Cancel" \
        3>&1 1>&2 2>&3)
    ...
    case $CHOICE in
        ...
        5) echo "Theme selection cancelled." ;;
        *) dialog --msgbox "No selection made. Exiting." 0 0 ;;
    esac
}
